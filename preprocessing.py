...
# Check bad channels
def Auto_Detect_Bad_Channels(raw,):
    
    data = raw.get_data(picks="eeg")
    
    std = data.std(axis=1)
    flat_chs = [raw.ch_names[i] for i, s in enumerate(std) if s < 1e-12]
    
    # findout high-var channels
    median_std = np.median(std)
    mad_std = np.median(np.abs(std - median_std))
    
    high_var_chs = [
        raw.ch_names[i] for i, s in enumerate(std) 
        if s > median_std + 6 * mad_std
    ]
    
    bad_channels = list(set(flat_chs + high_var_chs))
        
    print(f"[Info-python]: Findout the following bad channels: \n")
...
def Auto_Detect_Bad_Segments(
    ...
    if not isinstance(data, np.ndarray):
        ...
        for start in range(0, sdata.shape[1] - win + 1, step):
            stop = start + win
            seg = sdata[:, start:stop]

            ptp = np.ptp(seg, axis=1)  # peak-to-peak per channel

            if np.nanmax(ptp) > ptp_uv_thresh:
                bad_segments.append((start / sf, stop / sf))
